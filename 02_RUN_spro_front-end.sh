...
# as well as label files indicating the time label of speech frames in ./data/lbl/
	...
	CMD_NORM_E="/data/jiayuheng/score/ALIZE_file/LIA_RAL/bin/NormFeat --config cfg/NormFeat_energy_SPro.cfg --inputFeatureFilename data/dataPure0.lst --featureFilesPath  data/prm/"
	echo $CMD_NORM_E
	$CMD_NORM_E
        echo "End normalise energy : `date`\n "

        echo "Energy Detector : `date` "
        CMD_ENERGY="/data/jiayuheng/score/ALIZE_file/LIA_RAL/bin/EnergyDetector  --config cfg/EnergyDetector_SPro.cfg --inputFeatureFilename data/dataPure0.lst --featureFilesPath  data/prm/  --labelFilesPath  data/lbl/"
	echo $CMD_ENERGY
	$CMD_ENERGY
        echo "End energy detector : `date`\n "

        echo "Normalise Features : `date`"
        CMD_NORM="/data/jiayuheng/score/ALIZE_file/LIA_RAL/bin/NormFeat --config cfg/NormFeat_SPro.cfg --inputFeatureFilename data/dataPure0.lst --featureFilesPath data/prm/   --labelFilesPath  data/lbl/"
	echo $CMD_NORM
	$CMD_NORM
